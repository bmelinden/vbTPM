...
% M.L. 2012-11-22   : correct computation of bead correlation time (should
%                     not be corrected for downsampling)
% M.L. 2012-05-03   : fixed bug in handling of forbidden transitions
% M.L. 2012-02-15   : experimented with possibility to disallow remove certain
%                     transitions (between genuine states).
...
if(isfield(W,'E') && isfield(W,'Ec')) % then start with M step
    ...
    W.M.wA =  (W.PM.wA + W.E.wA).*(W.PM.wA>0);
    W.Mc.wA= (W.PMc.wA +W.Ec.wA).*(W.PMc.wA>0);    
    W.Mc.wR= (W.PMc.wR +W.Ec.wR).*(W.PMc.wR>0);    
    %% emission model
    W.M.n  = W.PM.n  + W.E.M;
    W.M.v  = W.PM.v  + W.E.V;
...
for k=1:W.N
    nz=find(W.PM.wA(k,:)>0);
    wAk0=sum(W.M.wA(k,nz));
    uAk0=sum(W.PM.wA(k,nz));
...
KL_Ak=zeros(1,W.N); 
for k=1:W.N
    nz=find(W.PMc.wA(k,:)>0);
    wAk0=sum(W.Mc.wA(k,nz));
    uAk0=sum(W.PMc.wA(k,nz));
    KL_Ak(k)=gammaln(wAk0)-gammaln(uAk0)-(wAk0-uAk0)*psi(wAk0)...
        -sum(gammaln(W.Mc.wA(k,nz))-gammaln(W.PMc.wA(k,nz))...
        -(W.Mc.wA(k,nz)-W.PMc.wA(k,nz)).*psi(W.Mc.wA(k,nz)));
end
...
W.est.cTC=[0 -1./log(W.Mc.mu(2:end))]/(X.fSample0); % real time units, correction for downsampling not needed
