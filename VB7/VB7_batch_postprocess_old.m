function gfig=VB7_batch_postprocess(runinputfile,xtrastates)
% gfig=VB7_batch_postprocess(runinputfile,xtrastates)
%
...
% to do-list:
% - plot RMS trace, viterbi path, and state lines for each trace (preload
%   them) automatically. Different colors for different state lines.
% - add legend to KB plot
...
% - add ability to align calibration states?
...
%% internal parameters
h.trjRMS_path_color{1}=[0 0 1]; % color of viterbi path 
...
if(0) % avoid excessive computing during test runs
    disp('using only a few traces, for debugging!')
   cal={cal{1:10}};
   trj={trj{1:10}};
end
for k=1:length(trj)
    for b=1:length(trj{k})
        if(~isempty(trj{k}{b}) && ~isempty(cal{k}))
            ...
            if(1)% pre-compute Viterbi path etc
                ...
                %W.Wtrj=VB7iterator(W.Wtrj,X);
                if(0) % ugly hack to increase prior strength
                    W.Wtrj.PM.wA=30*10*rowNormalize(W.Wtrj.PM.wA);
                    W.Wtrj=VB7iterator(W.Wtrj,X);
                end
...
% construct filename for analysis file
...
h.KBaxes=axes('parent',h.KBpanel,'position',[0.2 0.2 0.77 0.72],...
            'box','on');
...
% save the present state of analysis
...
VB7_inspectStates(W,x,W.est2.sViterbi,h.KBaxes);
...
if(newstate<=length(R.statelabels)) % then update visible states
    ...
    if(R.includetrj{h.k}(h.b))
        inp=inpolygon(trj{h.k}{h.b}.Wtrj.est.sKaverage,trj{h.k}{h.b}.Wtrj.est.sBaverage,XV,YV);
...
% produce a scatter plot of active states with text, and update the RMS
% plot too.
% scatterplot:
delete(h.KBdots);
delete(h.KBtext);
...
% to do: re plot the RMS plot, remember the old y-limits, and plot dashed
% horizontal lines for each state
...
if(R.includetrj{h.k}(h.b))
    ...
    set([h.trjRMS_line h.trjRMS_rms h.trjRMS_path],'color',get(h.KBpanel,'backgroundcolor'))
end

if(R.includetrj{h.k}(h.b)==false)
    set([h.KBaxes h.RMSaxes h.RMSfig],'color',get(h.KBpanel,'backgroundcolor'))
else
    set([h.KBaxes h.RMSaxes h.RMSfig],'color','w')
end
end
...
h.trjRMS_path(2)=plot((1:h.downSample:T)/h.fSample,RMS(h.sMaxP{h.k}{h.b}),...
    'color',h.trjRMS_path_color{2},'linewidth',2,'linestyle','--');
% plot RMS data
...
legend('Viterbi path','M.L. states','RMS data',4)
box on
end
