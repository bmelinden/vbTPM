...
\texttt{PIDprefix\_results.mat} (\texttt{PIDprefix} is a string set in
the runinput file), in the \texttt{target\_path} folder (see also
table tables \ref{tab:caltrj}).
...
\texttt{target\_path/PIDprefix\_results\_analyzed.mat} (with
\texttt{PIDprefix} read from the runinput file), and factorial models
are similarly written to
\texttt{target\_path/PIDprefix\_results\_analyzed.mat\_VB7kin.mat}.

For details on how vbTPM represents the models etc., we refer to
section \ref{sec:notation}. In particular, the fields of the various
results files are explained in tables \ref{tab:caltrj},
\ref{tab:results}, and \ref{tab:results}
